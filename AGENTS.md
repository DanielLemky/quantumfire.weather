...
- This plugin ID is `quantumfire.weather`. Keep it consistent in `manifest.json` and `omalaunch.json`.
...
- Finish each implementation turn by enabling the plugin with `omarchy plugin enable quantumfire.weather` so the user can review it.
